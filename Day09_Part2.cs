...
var hists = File.ReadAllLines("D:\\input.txt").Select(l => Regex.Match(l, @"(?:(-?\d+)\s*)+").Groups[1].Captures.Select(c => long.Parse(c.Value)).Reverse().ToList()).ToList();
